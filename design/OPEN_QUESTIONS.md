...
- Data quality — near-duplicate source records (found in Phase 0): AntiviralDB has both "Influenza Virus" and "Influenza virus" (distinct source subjects, differing only in case). Kept as 2 distinct harmonized records (faithful; `canonical_uri` uses the exact, unique source name). Decide whether near-duplicates should be merged — a domain call that risks losing each entry's distinct nested payload. Likely recurs across sources; the harmonizer must NEVER silently collapse them (a lowercasing slug did, and Globus would then overwrite one record at ingest, keyed on `subject`).
